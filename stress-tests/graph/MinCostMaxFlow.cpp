...
// #include "mcmfnew.h"
#include <bits/extc++.h>
#include "../../content/graphs/MinCostMaxFlow.h"
#include <cassert>
...
bool hasNegativeCycle(MCMF& mcmf) {
	vector<ll> dist(mcmf.n, 0);
	for(int iter = 0; iter < mcmf.n; iter++) {
		for(int u = 0; u < mcmf.n; u++) {
			for(int idx : mcmf.adj[u]) {
				auto [v, cap, flow, cost] = mcmf.edges[idx];
				if(flow < cap && dist[v] > dist[u] + cost) {
					dist[v] = dist[u] + cost;
					if(iter == mcmf.n - 1) return true;
				}
			}
		}
	}
